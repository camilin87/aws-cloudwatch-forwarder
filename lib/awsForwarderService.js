...
module.exports = function (cloudWatchLogs) {
    ...
    return {
        ...
        send: logLines => {
            return promise.create((fulfill, reject) => {
                ...
                logLines = (logLines || []).filter(l => l.message)
                if (logLines.length === 0){
                    fulfill()
                    return
                }

                logLines = escapeLineColors(logLines)
                ...
                logDebugMessage("cloudWatchLogs.putLogEvents, length=", logLines.length, "logData=", logData)
    ...
    function escapeLineColors(logLines){
        logLines.forEach(l => {
            l.message = l.message.replace(/\[\d+m/g, "")
        })
        return logLines
    }
}